...
#define TEST_FILE	L"mb1.bmp"
#define OUT_FILE	L"out.bmp"
...
void perform_test(FIBITMAP* bid);
...
int main()
{
	...
	// load image
	//bitmap = FreeImage_LoadU(FIF_JPEG, TEST_FILE, JPEG_DEFAULT);
	bitmap = FreeImage_LoadU(FIF_BMP, TEST_FILE, BMP_DEFAULT);
	if (!bitmap)
	{
		printf("cannot load image, exit...\n");
		exit(-1);
	}
...
void perform_test(FIBITMAP* dib)
{
	...
	else if ((image_type == FIT_BITMAP) && (FreeImage_GetBPP(dib) == 24)) 
	{
		...
		if (! FreeImage_SaveU(FIF_BMP, out_dib, OUT_FILE, 0))
		{
			printf("save new image failed\n");
		}
	}
}
