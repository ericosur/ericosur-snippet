...
# config
	...
	($code, $text) = &get_line($skt);
	die ("Unknown welcome string: '$code $text'\n") if ($code != 220);
	$ehlo_ok -- if ($text !~ /ESMTP/);

	# Send EHLO
	&say_hello($skt, $ehlo_ok, $hello_host, \%features) or exit (1);
...
sub run_smtp($)
{
	...
	if ((defined ($features{'STARTTLS'})
		|| defined ($features{'TLS'}))
		&& $starttls_ok)
	{
		printf ("Starting TLS...\n") if ($verbose >= 1);
	...
	if (defined ($datasrc))  {
		...
		printf"%d: there is no datasrc to send\n";
	}
...
=pod
need the following variables defined
...
sub get_config
{
	# read config from these files
	my @ini = qw(mail.conf mailconf.ini);

LOADINI:
	foreach my $inifile (@ini)  {
		print "load $inifile ...\n";
		if (not -e $inifile)  {
			next LOADINI;
		}

		if ( $inifile =~ m/\.conf$/ )  {	# use eval to load config
			open my $fh, $inifile or die;
			while ( <$fh> )  {
				next if /^$/;
				next if /^#/;
				s/\n//;
		...
		elsif ( $inifile =~ m/\.ini/ )  { # load as ini
			die "not implemented yet!";
		}
		else  {
			die "did not load config!";
		}
	}
}
