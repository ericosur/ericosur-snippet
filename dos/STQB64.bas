...
0001 REM THE VENERABLE STAR TREK COMPUTER GAME -- ANTTI J YLIKOSKI 12/19/2010
0002 REM ENTERED, MODIFIED AND DEBUGGED BY ANTTI J YLIKOSKI
0003 REM ORIGINALLY FROM: THE BEST OF CREATIVE COMPUTING, VOL 1,
0004 REM EDITED BY DAVID H. AHL, CREATIVE COMPUTING PRESS,
0005 REM P.O.BOX 789-M, MORRISTOWN, N. J. 07960, USA
0006 REM ISBN 0-916688-01-1, (C) 1976 BY CREATIVE COMPUTING
0007 REM
0008 REM THE QB64 VERSION -- SOME IDIOSYNCRACIES FIXED
0009 REM
0010 REM [VERSION "STREK7", 1/12/75 RCL]
0012 REM EDITED AND DEBUGGED BY ANTTI J. YLIKOSKI 02/10/2010 --
0014 REM THE PROGRAM EXHIBITS THE CONTROL STRUCTURE AFFECTIONATELY
0018 REM CALLED A "RAT'S NEST".
0020 REM IT PROBABY STEMS FROM THE ERA OF THE GOTO AND THE GOSUB
0025 REM ONCOLOGICAL SURGERY WOULD HAVE BEEN POSSIBLE BUT ANYWAY:
0027 REM "IF IT AIN'T BROKEN, DON'T FIX IT!"
0029 REM
0030 REM
0040 REM ***     *** STAR TREK ***
...
0090 REM ***   MODIFICATIONS TO THE LATTER (PLUS DEBUGGINGS) BY
0100 REM ***      BOB LEEDOM -- APRIL & DECEMBER 1974,
0110 REM *** WITH A LITTLE HELP FROM HIS FRIENDS...
0120 REM *** COMMENTS, EPITETHS, AND SUGGESTIONS SOLICITED (IN 1975)
0130 REM *** ADDRESS TO: R. C. LEEDOM
0140 REM ***    WESTINGHOUSE DEFENSE & ELECTRONIC SYSTEMS CNTR.
0150 REM ***    BOX 746, M. S. 338 BALTIMORE, MD 21283, USA
0152 REM
0153 REM ORIGINALLY CONVERTED TO THE FREEBASIC, SEE
0154 REM   HTTP://WWW.FREEBASIC.NET
0155 REM AND THE DOCUMENTATION
0156 REM   HTTP://WWW.FREEBASIC.NET/WIKI/
0157 REM BY ANTTI J YLIKOSKI 10-16-2010
0158 REM
0159 REM QB64 VERSION BY ANTTI J. YLIKOSKI 12-19-2010
0160 REM START THE QB64.EXE, OPEN THE PROGRAM FILE AND EXECUTE "RUN"
0161 REM TO ENJOY THE GAME -- GOOD LUCK, ADMIRAL!!!!!
0164 REM
0165 REM SEE http://www.qb64.net/
0166 REM
0167 REM EXISTING BUGS: THE CALCLULATION OF THE PHOTON TORPEDO COURSE
0168 REM IS PARTIALLY FLAWED.
0169 REM
0172 RANDOMIZE TIMER
0175 PRINT TAB(15); "* * * STAR TREK * * *"
0180 PRINT
0190 PRINT "DO YOU NEED INSTRUCTIONS (YES/NO)";
0200 DIM A$
0210 INPUT A$
0220 IF UCASE$(A$) = "YES" THEN GOSUB 10000
0240 REM THE PROGRAM BEGINS HERE....
0242 DIM Q9$
0244 DIM R9$
0246 DIM S4$
0250 DIM Z$
...
0278 FOR I=1 TO 72
    ...
    0310    LET S$=S$+" "
0320 NEXT I
0322 REM Z$ IS SO TO SPEAK A ZERO STRING -- FULL OF SPACES
0324 REM Q$ + R$ + S$ ARE ALTOGETHER 3*72 = 216 CHARACTERS OF MEMORY
0326 REM THE VIDEO OF THE CURRENT QUADRANT ARE (3*8)*8 CHARACTERS, IE.
0327 REM 192 CHARACTERS, STORED IN THE STRINGS Q$+R$+S$.
0330 DIM G(1 TO 8, 1 TO 8) AS INTEGER
0331 DIM C(1 TO 9, 1 TO 2) AS INTEGER
0332 DIM K(1 TO 3, 1 TO 3) AS INTEGER
0333 DIM N(1 TO 3) AS INTEGER
0334 DIM Z(1 TO 8, 1 TO 8) AS INTEGER
0340 DIM O1$
0341 DIM C$
0350 DIM A1$
0351 DIM T$
0360 DIM D$
0361 DIM O3$
0362 REM THE VARIABLE A1 IS THE GOOD LUCK FACTOR
0363 REM A1 = 1 <=> ALMOST IMPOSSIBLE GAME
0364 REM A1 = 3 <=> POSSIBLE GAME
0365 REM A1 = 5 <=> EASY GAME
0367 LET A1 = 3
0370 DIM C9 AS DOUBLE, A9 AS DOUBLE, W9 AS DOUBLE, D4 AS DOUBLE, D1 AS DOUBLE
0372 DIM P7 AS DOUBLE
0374 DIM A8 AS DOUBLE, X8 AS DOUBLE
0379 LET T = INT(RND(1)*20+20)*100
0380 LET T0 = T
0390 LET T9 = 30
0400 LET D0 = 0
0410 LET E0 = 3000
...
0460 LET S = 0
0465 LET F7 = 0
0470 REM AN AUXILIARY FUNCTION DEF FND(Z) SUBSTITUTED IN THE PROGRAM TEXT
0485 REM INITIALIZE ENTERPRISE'S POSITION
...
0650 LET C(8,2)=1
0660 LET C(9,2)=1
0670 DIM D(8) AS INTEGER
0680 FOR I=1 TO 8
    0690  LET D(I)=0
0700 NEXT I
0710 LET A1$="NSLPTSDCX"
0720 LET D$="WARP ENGINESS.R.SENSORS L.R.SENSORS PHASER CNTRL"
0730 LET D$=D$+"PHOTON TUBESDAMAGE CNTRLSHIELD CNTRLCOMPUTER    "
0740 LET G4$="III"
0750 LET G1$=" ANTARES. SIRIUS. RIGEL. DENEB. PROCYON. CAPELLA. VEGA. "
0760 LET G1$=G1$+"BETELGEUZE. CANOPUS. ALDEBARAN. ALTAIR. REGULUS. "
0770 LET G1$=G1$+"SAGITTARIUS. ARCTURUS. POLLUX. SPICA. "
0780 LET B9=0
0790 LET K9=0
0800 LET A1$="NSLPTSDCX"
0810 REM SET UP WHAT EXISTS IN THE GALAXY....
0820 FOR I=1 TO 8
    0830   FOR J=1 TO 8
        0840     LET R1=RND(1)
        0850     IF R1 > 0.98 THEN GOTO 0900
        0860     IF R1 > 0.95 THEN GOTO 0930
        0870     IF R1 > 0.8 THEN GOTO 0960
        0880     LET K3=0
        0890     GOTO 0980
        0900     LET K3=3
        0910     LET K9=K9+3
        ...
        0970     LET K9=K9+1
        0980     LET R1=RND(1)
        0990     IF R1 > 0.96 THEN GOTO 1020
        1000     LET B3=0
        1010     GOTO 1040
        1020     LET B3=1
        1030     LET B9=B9+1
...
2920 IF (RND(1) >= 0.8) THEN GOTO 3000
2925 REM CHANGED 0.6 TO 0.8 TO MAKE THE GAME LESS IMPOSSIBLE - A. J. Y.
...
2980 PRINT
2990 GOTO 3070
3000 LET D(R1)=D(R1)+(RND(1)*3.0+1.0)/A1
3005 REM ADDED THE "/A1" TO MAKE THE GAME LESS IMPOSSIBLE -- A. J. Y.
3010 PRINT
...
6340 INPUT A$
6350 IF UCASE$(A$)="AYE" THEN GOTO 0240
6360 GOTO 9250
...
6610 LET P=10
6612 REM THE VARIABLE P == # OF TORPEDOES
...
10130 PRINT "2. IF YOU SHOULD TYPE IN AN ILLEGAL COMMAND, YOUÄLL GET A SHORT"
...
30010 LET D4=ATN(ABS(X9)/ABS(A8))
